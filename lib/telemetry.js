...
function scrubText(value, max = TEXT_MAX) {
  ...
  const tokenFile = process.env.PHOVIA_TOKEN_FILE;
  if (tokenFile) {
    const escaped = String(tokenFile).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  ...
  text = text
    ...
    .replace(/\b(?:access_token|refresh_token|user_code|device_code)\b\s+["']?[^\s,;}"']+["']?/gi, '[redacted]')
